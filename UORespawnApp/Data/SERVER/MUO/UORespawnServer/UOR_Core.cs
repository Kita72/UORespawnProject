...
internal static class UOR_Core
{
    ...
    /// Note: VendorService functionality is now handled directly in OnServerStarted.
    /// </summary>
    ...
    private static void InitializeEvents()
    {
        ...
        EventSink.WorldSavePostSnapshot += EventSink_WorldSavePostSnapshot;
    ...
    private static void EventSink_WorldSavePostSnapshot(WorldSavePostSnapshotEventArgs e)
    {
        IsPaused = IsLocked;
    ...
    {
        if (m is PlayerMobile pm && !_RespawnerList.ContainsKey(pm.Serial))
        {
    ...
    private static void EventSink_Shutdown()
    {
    ...
    private static void EventSink_ServerCrashed(ServerCrashedEventArgs e)
    {
    ...
    internal static void RelogPlayers()
    {
        ...
        {
            if (ns.Mobile is PlayerMobile pm && !_RespawnerList.ContainsKey(pm.Serial))
            {
                _RespawnerList.Add(pm.Serial, new RespawnerEntity(pm));
    ...
    internal static bool DropStaff(PlayerMobile pm)
    {
        ...
        if (_RespawnerList.ContainsKey(pm.Serial))
        {
            ...
            UOR_Utility.SendMsg(ConsoleColor.Yellow, $"UORespawn-[{pm.Name} Dropped]");
