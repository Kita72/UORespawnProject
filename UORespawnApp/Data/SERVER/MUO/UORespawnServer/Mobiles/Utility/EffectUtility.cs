...
internal static class EffectUtility
{
    ...
    {
        if (UOR_Utility.IsValidSpawn((int)m.Serial.Value, out m)){ return UOREffects.None; }

        var tile = UOR_Utility.GetTileName(m.Map, m.Location);

        if (tile == "water"){ m.CanSwim = true; }

        return tile switch
        {
            "water"                  => UOREffects.Wave,
            "sand" or "grass" or "snow" => UOREffects.Wind,
            "blood" or "jungle"      => UOREffects.Mist,
            "swamp"                  => UOREffects.Poison,
            "forest"                 => UOREffects.Smoke,
            "marble"                 => UOREffects.Magic,
            _                        => UOREffects.None
        };
    }
    ...
    internal static void TryRunEffect(Mobile m, UOREffects effect)
    {
        if (effect != UOREffects.None && Utility.RandomDouble() < 0.50)
        {
            ...
            if (m is PlayerMobile p)
            {
                ...
                {
                    int healAmt = Utility.Random(p.Hits / 10);
                    p.Heal(healAmt);
                    p.SendMessage(42, $"You gained {healAmt} hitpoints!");
                }
                else
                {
                    int dmgAmt = Utility.Random(p.Hits / 10);
                    p.Damage(dmgAmt);
                    p.SendMessage(42, $"You lost {dmgAmt} hitpoints!");
                }
            }
            else
            {
                ...
                foreach (var mob in mobs)
                {
                    if (mob.Mobile is PlayerMobile pm)
                    {
                        if (effect == UOREffects.Glow || effect == UOREffects.Confetti || effect == UOREffects.Magic)
                        {
                            int healAmt = Utility.Random(pm.Hits / 10);
                            pm.Heal(healAmt);
                            pm.SendMessage(42, $"You gained {healAmt} hitpoints!");
                        }
                        else
                        {
                            int dmgAmt = Utility.Random(pm.Hits / 10);
                            pm.Damage(dmgAmt);
                            pm.SendMessage(42, $"You lost {dmgAmt} hitpoints!");
                        }
                    }
                }
            }
