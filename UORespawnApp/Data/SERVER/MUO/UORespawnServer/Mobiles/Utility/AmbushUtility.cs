...
internal static class AmbushUtility
{
    ...
    internal static Mobile GetUndeadAmbush()
    {
        if (Utility.RandomDouble() < 0.1)
        {
            if (Utility.RandomDouble() < 0.1)
            {
                return new SkeletalMage() { IsParagon = Utility.RandomDouble() < 0.01 };
            }
            else
            {
                return new SkeletalKnight() { IsParagon = Utility.RandomDouble() < 0.01 };
            }
        }
        else
        {
    ...
    internal static Mobile GetOrcAmbush()
    {
        if (Utility.RandomDouble() < 0.1)
        {
            ...
            {
                return new OrcBrute() { IsParagon = Utility.RandomDouble() < 0.01 };
            }
        }
        else
        {
            return new Orc() { IsParagon = Utility.RandomDouble() < 0.01 };
        }
    }
