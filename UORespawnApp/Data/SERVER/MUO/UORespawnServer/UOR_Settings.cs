...
internal static class UOR_Settings
{
    ...
    private static void ApplySetting(string key, string value)
    {
        switch (key.ToUpperInvariant())
        {
            // Scale Modifier
            case "SCALE_MOD":
                if (double.TryParse(value, out double scaleMod))
                {
                    SCALE_MOD = scaleMod;
                }
            ...
            case "SEARCH_INTERVAL":
                ...
                {
                    SEARCH_INTERVAL = searchInt;
                }

                break;
            case "PROCESS_INTERVAL":
                if (int.TryParse(value, out int processInt))
                {
                    PROCESS_INTERVAL = processInt;
                }

                break;
            case "VALIDATE_INTERVAL":
                if (int.TryParse(value, out int validateInt))
                {
                    VALIDATE_INTERVAL = validateInt;
                }

                break;
            case "TIMED_INTERVAL":
                if (int.TryParse(value, out int timedInt))
                {
                    TIMED_INTERVAL = timedInt;
                }
            ...
            case "MAX_RECYCLE_TYPE":
                if (int.TryParse(value, out int recycleType))
                {
                    MAX_RECYCLE_TYPE = recycleType;
                }

                break;
            case "MAX_SPAWN_CHECKS":
                if (int.TryParse(value, out int spawnChecks))
                {
                    MAX_SPAWN_CHECKS = spawnChecks;
                }

                break;
            case "MAX_QUEUE_SIZE":
                if (int.TryParse(value, out int queueSize))
                {
                ...
                break;
            case "MAX_STAT_SIZE":
                if (int.TryParse(value, out int statSize))
                {
                    MAX_STAT_SIZE = statSize;
                }
            ...
            // Spawn Limits
            case "MAX_SPAWN":
                if (int.TryParse(value, out int maxSpawnVal))
                {
                    MAX_SPAWN_VAL = maxSpawnVal;
                }

                break;
            case "MIN_RANGE":
                if (int.TryParse(value, out int minRangeVal))
                {
                    MIN_RANGE_VAL = minRangeVal;
                }

                break;
            case "MAX_RANGE":
                if (int.TryParse(value, out int maxRangeVal))
                {
                    MAX_RANGE_VAL = maxRangeVal;
                }

                break;
            case "MAX_CROWD":
                if (int.TryParse(value, out int maxCrowdVal))
                {
                    MAX_CROWD_VAL = maxCrowdVal;
                }
            ...
            // Spawn Chances
            case "CHANCE_WATER":
                if (double.TryParse(value, out double chanceWater))
                {
                    CHANCE_WATER = chanceWater;
                }

                break;
            case "CHANCE_WEATHER":
                if (double.TryParse(value, out double chanceWeather))
                {
                    CHANCE_WEATHER = chanceWeather;
                }

                break;
            case "CHANCE_TIMED":
                if (double.TryParse(value, out double chanceTimed))
                {
                    CHANCE_TIMED = chanceTimed;
                }

                break;
            case "CHANCE_COMMON":
                if (double.TryParse(value, out double chanceCommon))
                {
                    CHANCE_COMMON = chanceCommon;
                }

                break;
            case "CHANCE_UNCOMMON":
                if (double.TryParse(value, out double chanceUncommon))
                {
                    CHANCE_UNCOMMON = chanceUncommon;
                }

                break;
            case "CHANCE_RARE":
                if (double.TryParse(value, out double chanceRare))
                {
                    CHANCE_RARE = chanceRare;
                }
            ...
            case "ENABLE_SCALE_SPAWN":
                if (bool.TryParse(value, out bool enableScale))
                {
                    ENABLE_SCALE_SPAWN = enableScale;
                }

                break;
            case "ENABLE_RIFT_SPAWN":
                if (bool.TryParse(value, out bool enableRift))
                {
                    ENABLE_RIFT_SPAWN = enableRift;
                }

                break;
            case "ENABLE_TOWN_SPAWN":
                if (bool.TryParse(value, out bool enableTown))
                {
                    ENABLE_TOWN_SPAWN = enableTown;
                }

                break;
            case "ENABLE_GRAVE_SPAWN":
                if (bool.TryParse(value, out bool enableGrave))
                {
                    ENABLE_GRAVE_SPAWN = enableGrave;
                }
            ...
            case "ENABLE_VENDOR_SPAWN":
                if (bool.TryParse(value, out bool enableVendor))
                {
                    ENABLE_VENDOR_SPAWN = enableVendor;
                }

                break;
            case "ENABLE_VENDOR_NIGHT":
                if (bool.TryParse(value, out bool enableNight))
                {
                    ENABLE_VENDOR_NIGHT = enableNight;
                }

                break;
            case "ENABLE_VENDOR_EXTRA":
                if (bool.TryParse(value, out bool enableExtra))
                {
                    ENABLE_VENDOR_EXTRA = enableExtra;
                }
            ...
            case "ENABLE_SPAWN_EFFECTS":
                if (bool.TryParse(value, out bool enableEffects))
                {
                    ENABLE_SPAWN_EFFECTS = enableEffects;
                }
            ...
            // Debug Toggle
            case "ENABLE_DEBUG":
                if (bool.TryParse(value, out bool enableDebug))
                {
