...
internal static class SpawnManager
{
    ...
    private static void LoadBoxSpawnData()
    {
        try
        {
            ...
            using (BinaryReader reader = new(File.Open(BoxSpawnFile, FileMode.Open, FileAccess.Read)))
            {
    ...
    private static BoxEntity ReadBoxEntity(BinaryReader reader)
    {
        ...
        Rectangle2D spawnBox = new(x, y, width, height);

        BoxEntity entity = new(position, priority, spawnBox, weather, time)
        {
            WaterList = [.. ReadStringList(reader)],
            WeatherList = [.. ReadStringList(reader)],
            TimedList = [.. ReadStringList(reader)],
            CommonList = [.. ReadStringList(reader)],
            UnCommonList = [.. ReadStringList(reader)],
            RareList = [.. ReadStringList(reader)]
        };
    ...
    private static void LoadRegionSpawnData()
    {
        try
        {
            ...
            using (BinaryReader reader = new(File.Open(RegionSpawnFile, FileMode.Open, FileAccess.Read)))
            {
                int fileVersion = reader.ReadInt32();
                string versionString = reader.ReadString();
    ...
    private static RegionEntity ReadRegionEntity(BinaryReader reader, Map map)
    {
        ...
        RegionEntity entity = new(name, regionHandle, weather, time)
        {
            WaterList = [.. waterSpawns],
            WeatherList = [.. weatherSpawns],
            TimedList = [.. timedSpawns],
            CommonList = [.. commonSpawns],
            UnCommonList = [.. uncommonSpawns],
            RareList = [.. rareSpawns]
        };
    ...
    private static void LoadTileSpawnData()
    {
        try
        {
            ...
            using (BinaryReader reader = new(File.Open(TileSpawnFile, FileMode.Open, FileAccess.Read)))
            {
                int fileVersion = reader.ReadInt32();
                string versionString = reader.ReadString();
    ...
    private static TileEntity ReadTileEntity(BinaryReader reader)
    {
        ...
        TileEntity entity = new(name, weather, time)
        {
            WaterList = [.. ReadStringList(reader)],
            WeatherList = [.. ReadStringList(reader)],
            TimedList = [.. ReadStringList(reader)],
    ...
    internal static void LoadVendorSpawnData()
    {
        try
        {
            ...
            using (BinaryReader reader = new(File.Open(VendorSpawnFile, FileMode.Open, FileAccess.Read)))
            {
                int fileVersion = reader.ReadInt32();
                string versionString = reader.ReadString();
    ...
    private static bool ApplyRegionCommand(EditCommand command)
    {
        ...
        if (!int.TryParse(command.ExtraData.AsSpan(0, commaIndex), out int mapId))
        {
            UOR_Utility.SendMsg(ConsoleColor.Yellow, $"REGION COMMAND-[Invalid map ID in ExtraData]");
            return false;
        }

        string regionName = command.ExtraData[(commaIndex + 1)..];
    ...
    private static bool ApplyTileCommand(EditCommand command)
    {
        ...
        string tileName = command.ExtraData[(commaIndex + 1)..];
    ...
    private static List<string> GetSpawnList(ISpawnEntity entity, SpawnSection section)
    {
        ...
        {
            SpawnSection.Common => entity.CommonList,
            SpawnSection.Uncommon => entity.UnCommonList,
            SpawnSection.Rare => entity.RareList,
            SpawnSection.Water => entity.WaterList,
            SpawnSection.Weather => entity.WeatherList,
            SpawnSection.Timed => entity.TimedList,
            _ => null,
        };
    }
