...
{
    private static readonly List<string> _SessionLog = [];
    ...
    private static string GetLevelTag(ConsoleColor level)
    {
        return level switch
        {
            ConsoleColor.Yellow or ConsoleColor.DarkYellow => "DEBG",
            ConsoleColor.Red or ConsoleColor.DarkRed => "ERROR",
            _ => "INFO",
        };
    }
    ...
    internal static void FlushToFile(string reason)
    {
        ...
        {
            // Best-effort emergency log during shutdown
            try { File.AppendAllText(UOR_DIR.LOG_DEBUG_FILE + ".err", ex.Message + "\n"); } catch { }
        }
    }
}
