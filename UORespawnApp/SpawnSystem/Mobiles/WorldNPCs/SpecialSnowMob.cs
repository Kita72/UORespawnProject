    ...
    internal class SpecialSnowMob : BaseCreature
    {
        ...
        public override void OnAfterSpawn()
        {
            ...
            try
            {
                ...
                if (!allowed)
                {
                    string tile = SpawnSysUtility.TryGetWetName(Map, Location);

                    if (string.IsNullOrEmpty(tile) || tile == "NoName")
                        tile = SpawnSysTileInfo.GetTileName(Map.Tiles.GetLandTile(Location.X, Location.Y).ID);
